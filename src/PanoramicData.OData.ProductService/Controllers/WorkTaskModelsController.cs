...
namespace PanoramicData.OData.ProductService.Controllers
{
	public class WorkTaskModelsController : ODataController
	{
		...
		[EnableQuery]
		public IQueryable<WorkTaskModel> GetWorkTaskModels() => db.WorkTaskModels;

		// GET odata/WorkTaskModels(5)
		[EnableQuery]
		public SingleResult<WorkTaskModel> GetWorkTaskModel([FromODataUri] Guid key) => SingleResult.Create(db.WorkTaskModels.Where(workTaskModel => workTaskModel.Id == key));
		...
		private bool WorkTaskModelExists(Guid key) => db.WorkTaskModels.Count(e => e.Id == key) > 0;

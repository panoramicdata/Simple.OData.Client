...
namespace PanoramicData.OData.Client.Tests.Core
{
    ...
    {
		[Fact]
		public void ClientWithEmptySettings() => Assert.Throws<InvalidOperationException>(() => new ODataClient(new ODataClientSettings()));
		...
		[Fact]
		public void CtorWithHttpClientNoBaseAddressAndRelativeUri() => Assert.Throws<ArgumentException>(() => new ODataClientSettings(new HttpClient(), new Uri("api", UriKind.Relative)));
		...
		[Fact]
		public void CtorWithHttpClientAndRelativeBaseAddress() => Assert.Throws<ArgumentException>(() => new ODataClientSettings(new HttpClient { BaseAddress = new Uri("abc", UriKind.Relative) }));
		...
		[Fact]
		public void CtorWithHttpClientAndBaseAddressAndAbsoluteUrl() => Assert.Throws<ArgumentException>(() => new ODataClientSettings(new HttpClient { BaseAddress = new Uri("http://localhost") }, new Uri("http://localhost/api")));
	}
}

...
namespace PanoramicData.OData.Client.Tests
{
	...
	public class MockingRequestExecutor
	{
		...
		private static bool IsMetadataRequest(HttpRequestMessage request) => request.RequestUri.LocalPath.EndsWith(ODataLiteral.Metadata);
		...
		private string AdjustContent(string content) => AdjustBatchIds(
				AdjustNewLines(
				AdjustBaseUrl(
		...
		private static string AdjustNewLines(string content) => content.Replace("\r\n", "\n");

		private static string AdjustBaseUrl(string content) => _regexBaseUrl.Replace(content, "http://localhost/");
	...
	public static partial class ODataClientSettingsExtensionMethods
	{
		...
		private static string GetMockDataPathBase(string testMethodName) => Path.Combine(MockDataDir, testMethodName);

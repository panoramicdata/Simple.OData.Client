...
namespace PanoramicData.OData.Client.Adapter;
...
public class MetadataCache : IMetadata
{
	...
	public EntityCollection GetEntityCollection(string collectionPath) => ec.GetOrAdd(collectionPath, x => metadata.GetEntityCollection(x));
	...
	public EntityCollection GetDerivedEntityCollection(EntityCollection baseCollection, string entityTypeName) =>
		...
		metadata.GetDerivedEntityCollection(baseCollection, entityTypeName);

	public EntityCollection NavigateToCollection(string path) => nav.GetOrAdd(path, x => metadata.NavigateToCollection(x));

	public EntityCollection NavigateToCollection(EntityCollection rootCollection, string path) =>
		// Can't easily cache as the key would be a collection
		metadata.NavigateToCollection(rootCollection, path);

	public string GetEntityCollectionExactName(string collectionName) => ecen.GetOrAdd(collectionName, x => metadata.GetEntityCollectionExactName(x));

	public bool EntityCollectionRequiresOptimisticConcurrencyCheck(string collectionName) => roc.GetOrAdd(collectionName, x => metadata.EntityCollectionRequiresOptimisticConcurrencyCheck(x));

	public string GetEntityTypeExactName(string collectionName) => eten.GetOrAdd(collectionName, x => metadata.GetEntityTypeExactName(x));
	...
	public string GetLinkedCollectionName(string instanceTypeName, string typeName, out bool isSingleton) =>
		...
		metadata.GetLinkedCollectionName(instanceTypeName, typeName, out isSingleton);

	public string GetQualifiedTypeName(string typeOrCollectionName) => qtn.GetOrAdd(typeOrCollectionName, x => metadata.GetQualifiedTypeName(x));

	public bool IsOpenType(string collectionName) => ot.GetOrAdd(collectionName, x => metadata.IsOpenType(x));

	public bool IsTypeWithId(string typeName) => tid.GetOrAdd(typeName, x => metadata.IsTypeWithId(x));

	public IEnumerable<string> GetStructuralPropertyNames(string collectionName) => spns.GetOrAdd(collectionName, x => metadata.GetStructuralPropertyNames(collectionName).ToList());

	public bool HasStructuralProperty(string collectionName, string propertyName) => sp.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.HasStructuralProperty(collectionName, propertyName));

	public string GetStructuralPropertyExactName(string collectionName, string propertyName) => spen.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.GetStructuralPropertyExactName(collectionName, propertyName));

	public string GetStructuralPropertyPath(string collectionName, params string[] propertyNames) => spp.GetOrAdd(string.Join("/", propertyNames), x => metadata.GetStructuralPropertyPath(collectionName, propertyNames));

	public IEnumerable<string> GetDeclaredKeyPropertyNames(string collectionName) => dkpns.GetOrAdd(collectionName, x => metadata.GetDeclaredKeyPropertyNames(collectionName).ToList());

	public IEnumerable<string> GetNavigationPropertyNames(string collectionName) => npn.GetOrAdd(collectionName, x => metadata.GetNavigationPropertyNames(collectionName).ToList());

	public IEnumerable<IEnumerable<string>> GetAlternateKeyPropertyNames(string collectionName) => akpns.GetOrAdd(collectionName, x => metadata.GetAlternateKeyPropertyNames(collectionName).Select(y => (IList<string>)y.ToList()).ToList());

	public bool HasNavigationProperty(string collectionName, string propertyName) => np.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.HasNavigationProperty(collectionName, propertyName));

	public string GetNavigationPropertyExactName(string collectionName, string propertyName) => npen.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.GetNavigationPropertyExactName(collectionName, propertyName));

	public string GetNavigationPropertyPartnerTypeName(string collectionName, string propertyName) => nppt.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.GetNavigationPropertyPartnerTypeName(collectionName, propertyName));

	public bool IsNavigationPropertyCollection(string collectionName, string propertyName) => npc.GetOrAdd($"{collectionName}/{propertyName}", x => metadata.IsNavigationPropertyCollection(collectionName, propertyName));

	public string GetFunctionFullName(string functionName) => ffn.GetOrAdd(functionName, x => metadata.GetFunctionFullName(x));

	public EntityCollection GetFunctionReturnCollection(string functionName) => frc.GetOrAdd(functionName, x => metadata.GetFunctionReturnCollection(x));

	public string GetFunctionVerb(string functionName) => fv.GetOrAdd(functionName, x => metadata.GetFunctionVerb(x));

	public string GetActionFullName(string actionName) => afn.GetOrAdd(actionName, x => metadata.GetActionFullName(x));

	public EntityCollection GetActionReturnCollection(string functionName) => arc.GetOrAdd(functionName, x => metadata.GetActionReturnCollection(functionName));

...
public abstract class CommandFormatterBase : ICommandFormatter
{
	...
	protected string EscapeUnescapedString(string text) => text.ToCharArray().Intersect(ReservedUriCharacters.ToCharArray()).Any()
	...
	protected string FormatExpandItem(KeyValuePair<string, ODataExpandOptions> pathWithOptions, EntityCollection entityCollection) => FormatNavigationPath(entityCollection, pathWithOptions.Key);
	...
	protected static IEnumerable<KeyValuePair<string, ODataExpandOptions>> FlatExpandAssociations(
		IEnumerable<KeyValuePair<ODataExpandAssociation, ODataExpandOptions>> associations) => associations

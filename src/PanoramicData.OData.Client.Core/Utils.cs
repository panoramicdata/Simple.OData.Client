...
internal static class Utils
{
	...
	public static Stream StringToStream(string text) => new MemoryStream(Encoding.UTF8.GetBytes(text));

	public static Stream ByteArrayToStream(byte[] bytes) => new MemoryStream(bytes);
	...
	public static bool ContainsMatch(IEnumerable<string> actualNames, string requestedName, INameMatchResolver matchResolver) => actualNames.Any(x => matchResolver.IsMatch(x, requestedName));

	public static bool AllMatch(IEnumerable<string> subset, IEnumerable<string> superset, INameMatchResolver matchResolver) => subset.All(x => superset.Any(y => matchResolver.IsMatch(x, y)));
	...
	public static Exception NotSupportedExpression(Expression expression) => new NotSupportedException($"Not supported expression of type {expression.GetType()} ({expression.NodeType}): {expression}");
	...
	public static bool IsSystemType(Type type) => type.FullName.StartsWith("System.") ||
	...
	public static Task<T> GetTaskFromResult<T>(T result) => Task.FromResult(result);

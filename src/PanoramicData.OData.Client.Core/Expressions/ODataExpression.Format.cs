...
public partial class ODataExpression
{
	...
	private string FormatMappedOperator(ExpressionContext context, FunctionToOperatorMapping mapping) => mapping.Format(context, _functionCaller, Function.Arguments);
	...
	private string FormatToStringFunction(ExpressionContext context) => _functionCaller.Reference != null
	...
	private string FormatOperator(ExpressionContext context) => _operator switch
	{
	...
	private string FormatCallerReference() => _functionCaller.Reference.Replace(".", "/");

	private int GetPrecedence(ExpressionType op) => op switch
	{
	...
	private string FormatScope(string text, ExpressionContext context) => string.IsNullOrEmpty(context.ScopeQualifier)
			? text

...
internal class Session : ISession
{
	...
	public void Trace(string message, params object[] messageParams) => Settings.OnTrace?.Invoke(message, messageParams);
	...
	internal static Session FromSettings(ODataClientSettings settings) => new(settings);

	internal static Session FromMetadata(Uri baseUri, string metadataString) => new(baseUri, metadataString);
	...
	private EdmMetadataCache InitializeStaticMetadata(string metadata) => EdmMetadataCache.GetOrAdd(
			Settings.BaseUri.AbsoluteUri,
			uri => CreateMdc(uri, metadata));

	private async Task<EdmMetadataCache> InitializeMetadataCache(CancellationToken cancellationToken) => await EdmMetadataCache.GetOrAddAsync(
			Settings.BaseUri.AbsoluteUri,
			async uri =>
			{
	...
	private EdmMetadataCache CreateMdc(string key, string metadata) => new(key, metadata, TypeCaches.TypeCache(key, Settings.NameMatchResolver));
}

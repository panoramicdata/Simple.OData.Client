...
public class TypeCacheResolver
{
	...
	/// <returns></returns>
	public PropertyInfo GetMappedProperty(string propertyName) => (from t in MappedPropertiesWithNames where _nameMatchResolver.IsMatch(t.Item2, propertyName) select t.Item1).FirstOrDefault();

	public string GetMappedName(PropertyInfo propertyInfo) => (from t in MappedPropertiesWithNames where t.Item1 == propertyInfo select t.Item2).FirstOrDefault();

	public string GetMappedName(string propertyName) => (from t in MappedPropertiesWithNames where _nameMatchResolver.IsMatch(t.Item1.Name, propertyName) select t.Item2).FirstOrDefault();
	...
	public PropertyInfo GetDeclaredProperty(string propertyName) => TypeInfo.GetDeclaredProperty(propertyName);
	...
	public FieldInfo GetDeclaredField(string fieldName) => TypeInfo.GetDeclaredField(fieldName);

	public MethodInfo GetDeclaredMethod(string methodName) => TypeInfo.GetDeclaredMethod(methodName);

	public IEnumerable<ConstructorInfo> GetDeclaredConstructors() => TypeInfo.DeclaredConstructors.Where(x => !x.IsStatic);

	public ConstructorInfo GetDefaultConstructor() => GetDeclaredConstructors().SingleOrDefault(x => x.GetParameters().Length == 0);

	public TypeAttributes GetTypeAttributes() => TypeInfo.Attributes;

	public Type[] GetGenericTypeArguments() => TypeInfo.GenericTypeArguments;

	public bool IsTypeAssignableFrom(Type otherType) => TypeInfo.IsAssignableFrom(otherType.GetTypeInfo());

	public bool HasCustomAttribute(Type attributeType, bool inherit) => TypeInfo.GetCustomAttribute(attributeType, inherit) != null;
}

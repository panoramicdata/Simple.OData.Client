...
public class TypeCache : ITypeCache
{
	...
	public void Register<T>(string dynamicContainerName = "DynamicProperties") => Register(typeof(T), dynamicContainerName);
	...
	public bool IsDynamicType(Type type) => Resolver(type).IsDynamicType;

	/// <copydoc cref="ITypeCache.DynamicContainerName" />
	public string DynamicContainerName(Type type) => Resolver(type).DynamicPropertiesName;

	public PropertyInfo GetAnnotationsProperty(Type type) => Resolver(type).AnnotationsProperty;

	public IEnumerable<Type> GetDerivedTypes(Type type) => Resolver(type).DerivedTypes;

	/// <copydoc cref="ITypeCache.GetMappedProperties" />
	public IEnumerable<PropertyInfo> GetMappedProperties(Type type) => Resolver(type).MappedProperties;

	/// <copydoc cref="ITypeCache.GetMappedPropertiesWithNames" />
	public IEnumerable<Tuple<PropertyInfo, string>> GetMappedPropertiesWithNames(Type type) => Resolver(type).MappedPropertiesWithNames;

	/// <copydoc cref="ITypeCache.GetMappedProperty" />
	public PropertyInfo GetMappedProperty(Type type, string propertyName) => Resolver(type).GetMappedProperty(propertyName);

	/// <copydoc cref="ITypeCache.GetMappedName(Type, PropertyInfo)" />
	public string GetMappedName(Type type, PropertyInfo propertyInfo) => Resolver(type).GetMappedName(propertyInfo);

	/// <copydoc cref="ITypeCache.GetMappedName(Type, string)" />
	public string GetMappedName(Type type, string propertyName) => Resolver(type).GetMappedName(propertyName);

	/// <copydoc cref="ITypeCache.GetAllProperties" />
	public IEnumerable<PropertyInfo> GetAllProperties(Type type) => Resolver(type).AllProperties;

	/// <copydoc cref="ITypeCache.GetNamedProperty(Type, string)" />
	public PropertyInfo GetNamedProperty(Type type, string propertyName) => Resolver(type).GetAnyProperty(propertyName);

	/// <copydoc cref="ITypeCache.GetDeclaredProperties" />
	public IEnumerable<PropertyInfo> GetDeclaredProperties(Type type) => Resolver(type).DeclaredProperties;

	/// <copydoc cref="ITypeCache.GetDeclaredProperty" />
	public PropertyInfo GetDeclaredProperty(Type type, string propertyName) => Resolver(type).GetDeclaredProperty(propertyName);

	/// <copydoc cref="ITypeCache.GetAllFields" />
	public IEnumerable<FieldInfo> GetAllFields(Type type) => Resolver(type).AllFields;

	/// <copydoc cref="ITypeCache.GetAnyField" />
	public FieldInfo GetAnyField(Type type, string fieldName, bool includeNonPublic = false) => Resolver(type).GetAnyField(fieldName, includeNonPublic);

	/// <copydoc cref="ITypeCache.GetDeclaredFields" />
	public IEnumerable<FieldInfo> GetDeclaredFields(Type type) => Resolver(type).DeclaredFields;

	/// <copydoc cref="ITypeCache.GetDeclaredField" />
	public FieldInfo GetDeclaredField(Type type, string fieldName) => Resolver(type).GetDeclaredField(fieldName);

	/// <copydoc cref="ITypeCache.GetDeclaredMethod" />
	public MethodInfo GetDeclaredMethod(Type type, string methodName) => Resolver(type).GetDeclaredMethod(methodName);

	/// <copydoc cref="ITypeCache.GetDeclaredConstructors" />
	public IEnumerable<ConstructorInfo> GetDeclaredConstructors(Type type) => Resolver(type).GetDeclaredConstructors();

	/// <copydoc cref="ITypeCache.GetDefaultConstructor" />
	public ConstructorInfo GetDefaultConstructor(Type type) => Resolver(type).GetDefaultConstructor();

	/// <copydoc cref="ITypeCache.GetTypeAttributes" />
	public TypeAttributes GetTypeAttributes(Type type) => Resolver(type).GetTypeAttributes();

	/// <copydoc cref="ITypeCache.GetGenericTypeArguments" />
	public Type[] GetGenericTypeArguments(Type type) => Resolver(type).GetGenericTypeArguments();

	/// <copydoc cref="ITypeCache.IsAnonymousType" />
	public bool IsAnonymousType(Type type) => Resolver(type).IsAnonymousType;

	/// <copydoc cref="ITypeCache.IsGeneric" />
	public bool IsGeneric(Type type) => Resolver(type).IsGeneric;

	/// <copydoc cref="ITypeCache.IsValue" />
	public bool IsValue(Type type) => Resolver(type).IsValue;

	/// <copydoc cref="ITypeCache.IsEnumType" />
	public bool IsEnumType(Type type) => Resolver(type).IsEnumType;

	/// <copydoc cref="ITypeCache.GetBaseType" />
	public Type GetBaseType(Type type) => Resolver(type).BaseType;

	/// <copydoc cref="ITypeCache.IsTypeAssignableFrom" />
	public bool IsTypeAssignableFrom(Type type, Type otherType) => Resolver(type).IsTypeAssignableFrom(otherType);

	/// <copydoc cref="ITypeCache.HasCustomAttribute" />
	public bool HasCustomAttribute(Type type, Type attributeType, bool inherit) => Resolver(type).HasCustomAttribute(attributeType, inherit);

	/// <copydoc cref="ITypeCache.GetMappedName(Type)" />
	public string GetMappedName(Type type) => Resolver(type).MappedName;

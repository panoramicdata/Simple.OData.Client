...
public class ODataResponse
{
	...
	public T[] AsArray<T>() => AsEntries(false)
			.SelectMany(x => x.Values)
			.Select(x => TypeCache.Convert<T>(x))
			.ToArray();

	internal static ODataResponse FromNode(ITypeCache typeCache, ResponseNode node, IEnumerable<KeyValuePair<string, string>> headers) => new(typeCache)
	{
		Feed = node.Feed ?? new AnnotatedFeed(node.Entry != null ? new[] { node.Entry } : null),
		Headers = headers
	};

	internal static ODataResponse FromProperty(ITypeCache typeCache, string propertyName, object propertyValue) => FromFeed(typeCache, new[]
		{
				new Dictionary<string, object> { {propertyName ?? FluentCommand.ResultLiteral, propertyValue} }
			});

	internal static ODataResponse FromValueStream(ITypeCache typeCache, Stream stream, bool disposeStream = false) => FromFeed(typeCache, new[]
		{
				new Dictionary<string, object> { {FluentCommand.ResultLiteral, Utils.StreamToString(stream, disposeStream)} }
			});

	internal static ODataResponse FromCollection(ITypeCache typeCache, IList<object> collection) => new(typeCache)
	{
	...
	internal static ODataResponse FromBatch(ITypeCache typeCache, IList<ODataResponse> batch) => new(typeCache)
	{
		Batch = batch,
	};


	internal static ODataResponse FromStatusCode(ITypeCache typeCache, int statusCode, IEnumerable<KeyValuePair<string, string>> headers, Exception exception = null) => new(typeCache)
	{
	...
	internal static ODataResponse EmptyFeeds(ITypeCache typeCache) => FromFeed(typeCache, Array.Empty<Dictionary<string, object>>());

	private static ODataResponse FromFeed(ITypeCache typeCache, IEnumerable<IDictionary<string, object>> entries, ODataFeedAnnotations feedAnnotations = null) => new(typeCache)
	{

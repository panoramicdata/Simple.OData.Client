...
public class FluentCommand
{
	...
	internal ResolvedCommand Resolve(ISession session) => new(this, session);
	...
	public FluentCommand WithMedia(params ODataExpression[] properties) => WithMedia(properties.Select(x => x.Reference));
	...
	public FluentCommand Key(params object[] key) => Key(key.ToList());
	...
	public FluentCommand Select(params ODataExpression[] columns) => Select(columns.Select(x => x.Reference));
	...
	public FluentCommand OrderBy(params string[] columns) => OrderBy(SplitItems(columns).Select(x => new KeyValuePair<string, bool>(x, false)));

	public FluentCommand OrderBy(params ODataExpression[] columns) => OrderBy(columns.Select(x => new KeyValuePair<string, bool>(x.Reference, false)));
	...
	public FluentCommand ThenBy(params ODataExpression[] columns) => ThenBy(columns.Select(x => x.Reference).ToArray());

	public FluentCommand OrderByDescending(params string[] columns) => OrderBy(SplitItems(columns).Select(x => new KeyValuePair<string, bool>(x, true)));

	public FluentCommand OrderByDescending(params ODataExpression[] columns) => OrderBy(columns.Select(x => new KeyValuePair<string, bool>(x.Reference, true)));
	...
	public FluentCommand ThenByDescending(params ODataExpression[] columns) => ThenByDescending(columns.Select(x => x.Reference).ToArray());
	...
	public FluentCommand Media() => Media(MediaEntityLiteral);
	...
	public FluentCommand Media(ODataExpression expression) => Media(expression.Reference);
	...
	private IEnumerable<string> SplitItems(IEnumerable<string> columns) => columns.SelectMany(x => x.Split(',').Select(y => y.Trim()));

	private IEnumerable<KeyValuePair<string, bool>> SplitItems(IEnumerable<KeyValuePair<string, bool>> columns) => columns.SelectMany(x => x.Key.Split(',').Select(y => new KeyValuePair<string, bool>(y.Trim(), x.Value)));
}

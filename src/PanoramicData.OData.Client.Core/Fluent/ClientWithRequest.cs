    ...
    where T : class
{
	...
	public ODataRequest GetRequest() => _request;

	public IClientWithResponse<T> FromResponse(HttpResponseMessage responseMessage) => new ClientWithResponse<T>(_session, _request, responseMessage);

	public Task<IClientWithResponse<T>> RunAsync() => RunAsync(CancellationToken.None);

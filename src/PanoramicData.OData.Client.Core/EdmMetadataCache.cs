...
internal class EdmMetadataCache
{
	...
	public static void Clear(string key) => _instances.TryRemove(key, out _);

	public static EdmMetadataCache GetOrAdd(string key, Func<string, EdmMetadataCache> valueFactory) => _instances.GetOrAdd(key, valueFactory);
	...
	public IODataAdapter GetODataAdapter(ISession session) => session.Settings.AdapterFactory.CreateAdapterLoader(MetadataDocument, typeCache)(session);
}

...
public class TypeConverter : ITypeConverter
{
	...
	public void RegisterTypeConverter<T>(Func<IDictionary<string, object>, object> converter) => RegisterTypeConverter(typeof(T), converter);

	/// <copydoc cref="ITypeConverter.RegisterTypeConverter{T}(Func{object, object})" />
	public void RegisterTypeConverter<T>(Func<object, object> converter) => RegisterTypeConverter(typeof(T), converter);
	...
	public bool HasDictionaryConverter<T>() => HasDictionaryConverter(typeof(T));

	/// <copydoc cref="ITypeConverter.HasDictionaryConverter(Type)" />
	public bool HasDictionaryConverter(Type type) => _dictionaryConverters.ContainsKey(type);

	/// <copydoc cref="ITypeConverter.HasObjectConverter{T}" />
	public bool HasObjectConverter<T>() => HasObjectConverter(typeof(T));

	/// <copydoc cref="ITypeConverter.HasObjectConverter(Type)" />
	public bool HasObjectConverter(Type type) => _objectConverters.ContainsKey(type);

	/// <copydoc cref="ITypeConverter.Convert{T}(IDictionary{string, object})" />
	public T Convert<T>(IDictionary<string, object> value) => (T)Convert(value, typeof(T));

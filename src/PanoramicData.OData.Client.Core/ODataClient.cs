...
public partial class ODataClient : IODataClient
{
	...
	/// </summary>
	public static void ClearMetadataCache() => EdmMetadataCache.Clear();
	...
	/// </returns>
	public IBoundClient<IDictionary<string, object>> For(string collectionName) => GetBoundClient().For(collectionName);
	...
	/// The fluent OData client instance.
	/// </returns>
	public IBoundClient<ODataEntry> For(ODataExpression expression) => new BoundClient<ODataEntry>(this, Session).For(expression);
	...
	public IBoundClient<T> For<T>(string collectionName = null)
		where T : class => new BoundClient<T>(this, Session).For(collectionName);
	...
	public IUnboundClient<object> Unbound() => GetUnboundClient<object>();
	...
	public IUnboundClient<T> Unbound<T>()
		where T : class => GetUnboundClient<T>();

	private BoundClient<IDictionary<string, object>> GetBoundClient() => new(this, Session);

	private UnboundClient<T> GetUnboundClient<T>()
		where T : class => new(this, Session);
	...
	public void UpdateRequestHeaders(Dictionary<string, IEnumerable<string>> headers) => _settings.BeforeRequest += (request) =>
																					   {

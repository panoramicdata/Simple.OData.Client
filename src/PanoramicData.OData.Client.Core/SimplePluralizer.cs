...
internal class SimplePluralizer : IPluralizer
{
	...
	public string Pluralize(string noun) => AdjustCase(ToPluralInternal(noun), noun);

	public string Singularize(string noun) => AdjustCase(ToSingularInternal(noun), noun);

	public bool IsNounPluralOfNoun(string plural, string singular) => string.Compare(ToSingularInternal(plural), singular, StringComparison.OrdinalIgnoreCase) == 0;

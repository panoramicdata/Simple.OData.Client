...
namespace PanoramicData.OData.Client.Tests
{
    ...
    public abstract class TripPinTests : TripPinTestBase
    {
		...
		[Fact]
		public async Task UpdateMeGender_PreconditionRequired() => await AssertThrowsAsync<InvalidOperationException>(async () =>
																 {
																	 await _client
																		 .For<Person>("Me")
		...
		[Fact]
		public async Task ShareTrip() => await _client
				.For<Person>()
				.Key("russellwhyte")
				.Action("ShareTrip")

...
namespace PanoramicData.OData.Client.Tests
{
    public abstract class TestBase : IDisposable
    {
		...
		protected ODataClientSettings CreateDefaultSettingsWithNameResolver(INameMatchResolver nameMatchResolver) => new(_serviceUri)
		{
			PayloadFormat = _payloadFormat,
			IgnoreResourceNotFoundException = true,
			OnTrace = (x, y) => Console.WriteLine(string.Format(x, y)),
		...
		protected IODataClient CreateClientWithDefaultSettings() => new ODataClient(CreateDefaultSettings());

		protected IODataClient CreateClientWithNameResolver(INameMatchResolver nameMatchResolver) => new ODataClient(CreateDefaultSettingsWithNameResolver(nameMatchResolver));

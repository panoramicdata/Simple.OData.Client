...
public class ActionProvider : IDataServiceActionProvider
{
	...
	public IDataServiceInvokable CreateInvokable(DataServiceOperationContext operationContext, ServiceAction serviceAction, object[] parameters) => new ActionInvokable(operationContext, serviceAction, _context, parameters, _marshaller);

	public IEnumerable<ServiceAction> GetServiceActions(DataServiceOperationContext operationContext) => GetActions(operationContext);

	public IEnumerable<ServiceAction> GetServiceActionsByBindingParameterType(DataServiceOperationContext operationContext, ResourceType resourceType) => GetActions(operationContext).Where(a => a.Parameters.Count > 0 && a.Parameters.First().ParameterType == resourceType);

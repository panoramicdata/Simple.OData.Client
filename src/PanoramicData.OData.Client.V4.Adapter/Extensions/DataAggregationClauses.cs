...
internal class FilterClause : IDataAggregationClause
{
	...
	internal void Append(string filter) => _filter = $"{_filter} and {filter}";

	internal void Append(ODataExpression expression) => _filterExpression = _filterExpression && expression;
...
internal class AggregationClauseCollection<T> : IDataAggregationClause
{
	...
	internal void Add(AggregationClause<T> clause) => _clauses.Add(clause);

	public string Format(ExpressionContext context) => _clauses.Any()

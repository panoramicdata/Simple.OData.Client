...
public class DynamicDataAggregation
{
	...
	internal DataAggregationBuilder CreateBuilder() => _underlyingDataAggregationBuilder;

	private class DataAggregationBuilderHolder : DataAggregationBuilder
    {
		internal void Add(IDataAggregationClause dataAggregationClause) => DataAggregationClauses.Add(dataAggregationClause);

		internal IDataAggregationClause LastOrDefault(Func<IDataAggregationClause, bool> predicate = null) => DataAggregationClauses.LastOrDefault(predicate ?? (x => true));
	}
}

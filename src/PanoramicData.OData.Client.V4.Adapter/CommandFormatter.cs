...
public class CommandFormatter : CommandFormatterBase
{
	...
	protected override void FormatInlineCount(IList<string> commandClauses) => commandClauses.Add($"{ODataLiteral.Count}={ODataLiteral.True}");
	...
	private static ODataExpandAssociation MergeExpandAssociations(ODataExpandAssociation expandAssociation, string path) => MergeExpandAssociations(expandAssociation, ODataExpandAssociation.From(path)).First();
	...
	private bool HasMultipleSegments(string path) => path.Contains("/");

	private string FormatFirstSegment(string path) => path.Split('/').First();

	private string FormatSkipSegments(string path, int skipCount) => string.Join("/", path.Split('/').Skip(skipCount));
}

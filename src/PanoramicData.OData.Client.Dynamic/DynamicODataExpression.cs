...
public class DynamicODataExpression : ODataExpression, IDynamicMetaObjectProvider
{
	...
	public DynamicMetaObject GetMetaObject(Expression parameter) => new DynamicExpressionMetaObject(parameter, this);
	...
	private static IEnumerable<ConstructorInfo> GetConstructorInfo()
		=> _ctors ??= typeof(DynamicODataExpression).GetDeclaredConstructors().ToArray();

	private static ConstructorInfo CtorWithString => _ctorWithString
		??= GetConstructorInfo().Single(x =>
